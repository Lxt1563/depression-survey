...
These characteristics are objective in nature and are able to discriminate between populations that are healthy and those that are depressed.
But with the rapid advancement of deep learning techniques in recent years, speech auxiliary depression diagnosis has transitioned from hand-crafted acoustic features to a deep learning-based framework.
This family of approaches tackles two different tasks: depression classification and depression level regression.
%
...
In the early studies of speech auxiliary depression diagnosis, the main work is to learn acoustic features related with depression and explore feature set for better performance.
...
In the model, 238 audio feature descriptors extracted with openSMILE toolkit are first input into a CNN to learn high-level features, which are then fed to fully connected layer to predict the PHQ-8 score.
...
Hybrid networks are developed to gather more critical temporal information related to depression diagnosis.
...
In their study, CNN is used to extract detailed information from unprocessed waves.
...
Further, in order to capture multi-scale temporal information of speech signals, Du et al.~\cite{du2018bipolar} propose a novel speech-based approach, called IncepLSTM, which effectively integrates Inception module and LSTM on the feature sequence.
The Inception module, originally designed to widen the CNN architecture and expand the receptive field.
In order to avoid the over-fitting problem in a small amount of training samples, they apply the $L^{1}$ Regularizer to achieve a sparse version of IncepLSTM, ameliorating the generalization performance.
...
The transformer is suitable for very long sequence data.
...
Beisdes, The last important problem is that models so far are heavily restricted by the limited amount of depression data.
...
\begin{table*}[!htbp]
\centering
\caption{Performance of speech auxiliary depression diagnosis based on machine learning.}
\label{tab_speech}
