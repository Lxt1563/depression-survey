...
In the model, 238 audio feature descriptors extracted with the openSMILE toolkit are first input into a CNN to learn high-level features. These are then fed to a fully connected layer to predict the PHQ-8 score.
...
Moreover, speech samples from the DAIC-Woz dataset validate the models.
...
Their proposed model consists of the transformer encoder and the Embedding FC Block, where the transformer encoder extracts temporal information. The Embedding FC Block combines with a RELU activation layer, a dropout layer, and a fully-connected layer to extract the hidden embeddings representing each kind of feature. The embeddings from each kind of feature are fed to two FC blocks to perform depression prediction.
