...
The fact that depressed people typically speak more smoothly and monotonously than healthy people, who speak more rhythmically, with fewer pauses and times than depressed people, illustrates these distinctions \cite{pampouchidou2017facial,montgomery1979new}.
Furthermore, traditional speech auxiliary depression diagnosis methods design characteristics based on the variations between the two groups.
These characteristics are objective and can discriminate between healthy populations and those that are depressed.
Nevertheless, with the rapid advancement of deep learning techniques in recent years, speech auxiliary depression diagnosis is transitioned from hand-crafted acoustic features to a deep learning-based framework.
...
%In the early studies of speech auxiliary depression diagnosis, the main work is to learn acoustic features related to depression and explore the feature sets for better performance. The extracted hand-crafted audio features to detect depression might classify into four main groups: prosodic, voice quality, formant, and spectral features~\cite{france2000acoustical,moore2003analysis}.

In the early studies of speech auxiliary depression diagnosis, the main work is to learn acoustic features associated with depression in order to perform better. These features might classify into four main groups: prosodic, voice quality, formant, and spectral features~\cite{france2000acoustical,moore2003analysis}.


\emph{Prosodic features:} The prosodic features are computed from the speech waveform and include fundamental frequency, log of energy, jitter, and shimmer\cite{france2000acoustical}.
...
\emph{Formant features:} Formant features provide information regarding vocal tract resonance and pronunciation, representing physical vocal tract properties. As formant features, the first three formants are commonly employed.
...
%The above four features are regularly extracted in frames to generate frame-level Low-Level Descriptors.
...
As a result, many researchers develop and mix various acoustic features to build higher-performance auxiliary depression diagnosis models.
For example, Shankari et al.~\cite{shankayi2012identifying} extract three features from speech signals: prosodic, vocal tract spectrum, and glottal source. The results show that using features together leads to better results than using each category alone.
...
For the speech auxiliary depression diagnosis, CNN typically takes the acoustic feature or spectrogram as the input.
%(1) \emph{Acoustic feature-based}.
Well-known acoustic features, like MFCCs and Logarithmic Mel-spectrogram are fairly simple to extract and have a small number of dimensions which might be more suitable to a low-resource setting than raw signal.
Yang et al.~\cite{yang2017hybrid} present an acoustic features-based CNN model.
In the model, 238 audio feature descriptors are first input into a CNN to learn high-level features. These are then fed to a fully connected layer to predict the PHQ-8 score.
...
The fact that most of the input speech features are the amplitude spectrogram, which loses the phase spectrogram information, means that important information concerning depression is not present.
In order to fully utilize speech information, Fan et al.~\cite{fan2022csenet} use the complex spectrogram as the input feature. The complex spectrogram contains all of the speech information, both amplitude and phase spectrogram.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Several studies~\cite{le2017discretized,ma2016depaudionet} demonstrate that the temporal features of speech signals transmit basic information regarding emotional and mental disorders, which can be represented by RNNs and LSTMs.
%Several studies~\cite{le2017discretized,ma2016depaudionet} demonstrate that a speech signal's temporal features convey essential information regarding emotion and mental problems and use sequential classifiers such as RNN and LSTM.
%The RNN and the LSTM use feedback loops to keep information in 'memory' over time.
The LSTM outperforms the RNN, as it does better at avoiding the vanishing gradient problem and captures more extended temporal context information.
Mao et al.~\cite{mao2022prediction} use LSTM to estimate the depression severity of participants.
Also, they compare the bidirectional LSTM, which outperforms the traditional LSTM because the forward and backward networks combine the input sequence's forward and backward context information.
...
Hybrid networks are developed to gather more critical temporal information.
Some recent studies are combined a CNN feature extraction architecture with sequential LSTM, 
an approach named CNN-LSTM that can learn to recognize and synthesize sequential dynamics in speech. 
In the CNN-LSTM, the CNN acts as the trainable feature detector for the spatial signal.
It learns features that operate on a static spatial input (windows) while the LSTM receives a sequence of high-level representations to generate a description of the content.
Ma et al.~\cite{ma2016depaudionet} uses CNN to extract detailed information from unprocessed speech waves.
...
The transformer is suitable for very long sequence data and is studied extensively in speech auxiliary depression diagnosis during the past two years.
...
heir proposed model consists of the transformer encoder and the Embedding FC Block, where the transformer encoder extracts temporal information and the FC blocks perform depression prediction.
%%Their proposed model consists of the transformer encoder and the Embedding FC Block, where the transformer encoder extracts temporal information and the FC blocks perform depression prediction.
...
%The current two biggest obstacles to depression detection jobs are the small quantity of datasets and the lengthy input sequences.
Subsequently, Zhang~\cite{zhang2021depa} proposes an embedding method that can solve the limited amount of data while summarizing long sequences.
An encoder-decoder model is trained self-supervised to predict and reconstruct a centre spectrogram given a spectrogram context.
...
%To address these two challenges, Zhang~\cite{zhang2021depa} proposes an embedding transformer method that can solve the limited amount of data while summarizing long sequences.
%An encoder-decoder model is trained self-supervised to predict and reconstruct a centre spectrogram given a spectrogram context.
%Then, DEPA is extracted from a trained encoder model and fed into a depression detection bidirectional LSTM network. DEPA exhibits an excellent performance compared to other commonly used features.
...
Tabel~\ref{tab_speech} overviews the speech auxiliary depression diagnosis techniques' performance comparisons.
From the table we obtain the following three observations:


(1)
Comparative investigation of the abilities of various traditional classifiers in depression evaluation and prediction shows that the employment of a hybrid classifier utilizing GMM and SVM model gives the best overall classification results for speech auxiliary depression diagnosis~\cite{alghowinem2013comparative}.
This demonstrates that the two have complementary traits and that combining their advantages can significantly raise the recognition rate.


(2)
The performance of end-to-end model is better than the spectrogram-based deep model~\cite{srimadhur2020end}.
%Srimadhur et al. \cite{srimadhur2020end} experiment on depression detection using spectrogram-based CNN and deep end-to-end models. The optimal model for classifying the depressed state are determined after parameter tuning, comparison analysis, and evaluation of the two models. The results indicate that the performance of end to end model was better than the baseline models and spectrogram-based convolutional neural network model on the DAIC-WOZ dataset.
...
As it could solve the problems encountered with hand-crafted features, such as high threshold, labour cost and low feature utilization rate, deep learning  gradually becomes the leader in the field of machine
learning.
...
\toprule
\textbf{Paper}& \textbf{Dataset} & \textbf{Methods} & \textbf{F1 Score} & \textbf{RMSE} & \textbf{MAE} \\ \midrule
...
Al Hannai et al.~\cite{al2018detecting}& DAIC-WOZ & LSTM& 63.00 & 6.50 & 5.13         \\ \hline
Ma et al.~\cite{ma2016depaudionet}& DAIC-WOZ & CNN+LSTM & 52.00 & - &-  \\ \hline
Salekin et al.~\cite{salekin2018weakly} & DAIC-WOZ & LSTM & 90.10&-&-     \\ \hline
\multirow{2}{*}{Srimadhur et al.~\cite{srimadhur2020end}} & \multirow{2}{*}{DAIC-WOZ} & CNN & 78.00 &-&- \\  \cline{3-6}
 & & CNN(Spectrogram) & 66.00 &-&- \\ \hline
Zhang et al.~\cite{zhang2021depa}& DAIC-WOZ & Transformer & 94.00 & 5.73& 4.75     \\ \bottomrule
\end{tabular}
\end{threeparttable}}
\end{table}
