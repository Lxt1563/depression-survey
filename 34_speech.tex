...
Nevertheless, with the rapid advancement of deep learning techniques in recent years, speech depression recognition has transitioned from hand-crafted acoustic features to a deep learning-based framework.
...
In the early studies of speech depression recognition, the main work is to learn acoustic features related to depression and explore feature sets for better performance.
...
The glottal spectrum shows the condition of vocal function, while frequency and amplitude perturbation combined describe the stability of vocal fold vibration\cite{moore2007critical}.
\emph{Formant features:}Formant features provide information regarding vocal tract resonance and pronunciation efforts, which represent physical vocal tract properties. As formant features, the first three formants are commonly employed.
\emph{Spectral features:} Spectral features reflect the relationship between the shape change of the vocal tract and the articulator movement. PSD and Mel-scaleFrequency Cepstral Coefficients (MFCC) are the most widely utilized features.
...
Shankari et al.\cite{shankayi2012identifying} extract three categories of features from speech signals: prosodic, vocal tract spectrum, and glottal source. The results showed that using features together leads to better results than using each category alone.


In traditional speech depression recognition studies, SVM~\cite{nasir2016multimodal,gong2017topic,cummins2013spectro,helfer2013classification} and Gaussian
...
\subsubsection{Speech depression recognition based on deep learning}

Deep learning techniques have demonstrated their effectiveness in numerous speech-based applications.
In speech depression recognition, CNN, RNN, hybrid networks, and transformer networks are some of the frequently utilized deep classifier techniques.
...
Therefore, it can be used as a classifier for a speech depression recognition.
...
Moreover, speech samples from the audio-visual emotion challenge (AVEC) 2016 DAIC-Woz dataset were utilized for validating the models.
...
In order to make full use of speech information, Fan et al.~\cite{fan2022csenet} propose a complex squeeze-and-excitation network (CSENet), which uses the complex spectrogram as the input feature. The complex spectrogram contains all of the speech information, both amplitude and phase spectrogram.
Despite having good outcomes in speech depression recognition, conventional CNNs have the obvious drawback of being unable to capture lengthy temporal context information.
...
The Bi-LSTM outperforms the traditional LSTM because the forward and backward networks combine the input sequence's forward and backward context information.
...
Hybrid networks are developed to gather more critical temporal information about depression recognition.
...
The transformer is suitable for very long sequence data and has been studied extensively in speech depression recognition during the past two years.
...
It is the first transformer-based approach for speech depression recognition.
...
Then, DEPA is extracted from a trained encoder model and fed into a depression detection BLSTM network. DEPA exhibits an excellent performance compared to other commonly used features.
...
The performance comparisons based on traditional machine learning and deep learning are summarized in Table~\ref{tab_speech}.
From the table, we obtain the following three observations:

(1) With speech  depression recognition based on traditional machine learning,
...
Srimadhur\cite{srimadhur2020end} experiment on depression detection using spectrogram-based CNN and deep end-to-end models. Parameter tuning has been performed, comparative analysis has been carried out between two models, and the best model has been chosen for categorizing the depression state. The results indicated that the performance of end to end model was better than the baseline models and spectrogram-based convolutional neural network model on the DAIC-WOZ dataset.
...
& & & \textbf{F1} & \textbf{Precision} & \textbf{Recall} & \textbf{accuracy} & \textbf{RMSE}           & \textbf{MAE}\\ \hline
...
Cummins et al.~\cite{cummins2011investigation} & AVEC2013 & GMM  & -& -& -& 80 &-&- \\ \hline
Williamson et al.~\cite{williamson2013vocal}& AVEC2013 & GMM &- & -& - & -& 7.42 & 5.75  \\ \hline
Williamson et al.~\cite{williamson2014vocal}& AVEC2014  & GMM &-& -&- &- & 8.5 -& 6.52 \\ \hline
Jan et al.~\cite{jan2017artificial} & AVEC2014 & LR & -& - & -& -& 7.43 & 6.14  \\ \hline
Nasir et al.~\cite{nasir2016multimodal} & DAIC-WOZ& SVM& 63 &-& -& -&-& -      \\ \hline
...
Al Hannai et al.i~\cite{al2018detecting}& DAIC-WOZ & LSTM & 63& 71 & 56 &-& 6.50  & 5.13  \\ \hline
Dham et al.~\cite{dham2017depression}  & DAIC-WOZ & SVM & -& -& -& -& 6.32  & 4.4    \\ \hline
Othmani et al.~\cite{othmani2021towards}& DAIC-WOZ & CNN& -&-&-& 73.25& 0.467&- \\ \hline
Ma et al.~\cite{ma2016depaudionet}& DAIC-WOZ  & CNN+LSTM & 52 & 35 & 1 &- &- & -  \\ \hline
Salekin et al.~\cite{salekin2018weakly}& DAIC-WOZ & LSTM  & 90.1 & -& -& 90 & -& -    \\ \hline
\multirow{2}{*}{Srimadhur et al.~\cite{srimadhur2020end}}& \multirow{2}{*}{DAIC-WOZ}  & CNN & 78& -& -& - & -& -   \\ \cline{3-9}
& & CNN(Spectrogram)  & 66 & - &- &-&-&-   \\ \hline
Zhang et al.~\cite{zhang2021depa}& DAIC-WOZ & Transformer & 94 & 93& 76  &-& 5.73  & 4.75 \\ \hline
\end{tabular}}
\end{table*}
