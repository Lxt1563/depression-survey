...
\section{Depression recognition method based on social network data}
\label{sec_approach}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Its function is to maintain the basic framework of the test information retrieval system and create a data repository for researchers to use to develop comparable standards.
Since the first seminar was formed in 2000, the organization has held a meeting in Europe every September.
This meeting includes ten benchmarking labs reported results of their year long activities in overview talks and lab sessions, and eRisk is one of the tasks.
eRisk pilot task focuses on automatically detecting depression as early as possible from a users’ posts to Reddit, which promote the auxiliary diagnosis of depression based on social media data.

\subsubsection{Social network depression recognition based on traditional machine learning}
\label{sec_fquality}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The researchers extracted features such as sentiment, mood and writing behavior of users from different social networking platforms and used various machine learning models for depression prediction, such as SVM, Random forest, Naïve Bayes, Logistic regression, Decision Trees, etc~\cite{de2013predicting,losada2016test,2019Early,nadeem2016identifying,fatima2018analysis}.
...
Wang et al.~\cite{wang2013A} based on the 10 features of microblog users, including first person singular and plural pronouns, emoticons, interactions with others, behaviors and other features to build a detection model.
Shen et al.~\cite{shen2017depression} extracts 6 characteristic groups including social network, user profile, visual, emotional, topic-level and domain specific features. These features not only cover clinical depression standards, but also cover online behavior. Using these feature groups, the author proposes a multimodal depression dictionary learning model to detect depressed users.
...
Fatima et al.~\cite{fatima2018analysis} uses language style and sentiment information to find the most effective data dimensions by Linguistic Inquiry and Word Count by applying feature extraction, and describe the degree of depression expressed in the text as mild, moderate or severe according to the user's mood.
Wolohan et al.~\cite{wolohan2018detecting} uses Linguistic Inquiry and Word Count combined with ngram to diagnose depression.
...
\subsubsection{Social network depression recognition based on deep learning}
\label{sec_fquality}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Since deep learning technology can automatically construct text features, people have proposed  CNN, RNN and other models to solve the problem of depression analysis based on text~\cite{yates2017depression,shen2017depression,orabi2018deep,ive2018hierarchical}.
...
Shen~\cite{shen2018cross} et al. proposed a cross-domain DNN model with feature adaptive transformation and combination strategy to transfer relevant information to a heterogeneous domain, using sufficient Twitter data as the source domain and enhanced detection in some other target domain (e.g., Weibo).

CNN is a representative algorithm of deep learning.
With its powerful learning ability, it has been widely used in various fields, including text analysis in social networks.
...
LSTM is a special type of RNN that learns long-term dependent information and has also been widely used in depression detection based on social media texts. 
...
\multirow{2}{*}{Paper}               & \multicolumn{2}{c|}{Data}                                                                                                                                                              & \multirow{2}{*}{Method}            & \multicolumn{5}{c}{Metrics}                     \\
                                                                                                       & Media                                                                          & Dataset(D+H)                                                                                              &                            & Precision & Recall & F1-Score & Accuracy & AUC  \\
\hline
Choudhury et al.~\cite{de2013predicting}                                          & Twitter                                                                        & 476(171+305)                                                                                              & SVM                        & 0.74      & 0.63   & -        & 0.70     & -    \\
\hline
Tsugawa et al.~\cite{tsugawa2015recognizing}                                      & Twitter                                                                        & 209(81+121)                                                                                               & SVM                        & 0.64      & 0..43  & 0.52     & 0.69     & -    \\
\hline
Wolohan et al.~\cite{wolohan2018detecting}                                       & Reddit                                                                         & 12106(4947+7159)                                                                                          & SVM                        & -         & -      & 0.73     & 0.82     & 0.79 \\
\hline
Shatte et al.~\cite{shatte2019social}                                            & Reddit                                                                         & 365                                                                                                       & SVM                        & 0.67      & 0.68   & 0.67     & 0.66     & -    \\
\hline
Peng et al.~\cite{peng2019multi}                                                 & Weibo                                                                          & 276(141+135)                                                                                              & Multi-kernel SVM           & 0.76      & 0.77   & 0.76     & 0.83     & -    \\
\hline
Aldarwish et al.~\cite{ aldarwish2017predicting}                                 & \begin{tabular}[c]{@{}c@{}}Facebook,\\  LiveJournal,   \\ Twitter\end{tabular} & \begin{tabular}[c]{@{}c@{}}Posts(758+1374)\\  (489+1865)\\(826+1461)   \\ Total6773(2073+4700)\end{tabular}                                                          & SVM+Naïve Bayes           & 1.00      & 0.57   & -        & 0.63     & -    \\
\hline
Nadeem et al. ~\cite{nadeem2016identifying}                                      & Twitter                                                                        & \begin{tabular}[c]{@{}c@{}}CLPsych2015\\ 900(326+574)\end{tabular}             & Naïve Bayes                & 0.81      & 0.82   & 0.81     & 0.86     & 0.94 \\
\hline
Nadeem et al.~\cite{nadeem2016identifying}                                       & Twitter                                                                        & \begin{tabular}[c]{@{}c@{}}CLPsych2015\\ 900(326+574)\end{tabular}             & Logistic Regression        & 0.86      & 0.82   & 0.84     & 0.82     & 0.91 \\
%Dinkel et al$\sim$~\cite{ dinkel2019text}                                        & -                                                                              & DAIC-WOZ107                                                                                               & BGRU                       & 0.85      & 0.83   & 0.84     & 0.86     & -    \\
\hline
Eichstaedt et al.~\cite{ eichstaedt2018facebook }                                & Facebook                                                                       & 683(114+569)                                                                                              & Logistic regression        & -         & -      & -        & -        & 0.72 \\
\hline
Burdisso et al.~\cite{burdisso2019text}                                          & Reddit                                                                         & \begin{tabular}[c]{@{}c@{}}eRisk2017\\(83+403/52+349)\end{tabular}                                                                                                                                                                 & Incremental classification & 0.63      & 0.60   & 0.61     & -        & -    \\
\hline
Mariñelarena et al.~\cite{ marinelarena2017predicting   }                         & Reddit                                                                         & 486(83+403)                                                                                               & DNN                        & 0.85      & 0.81   & 0.83     & 0.94     & -    \\
\hline
Shen et al.~\cite{shen2018cross}                                                 & \begin{tabular}[c]{@{}c@{}}Twitter,\\ Weibo\end{tabular}                                                                                                                          & \begin{tabular}[c]{@{}c@{}}(1394+1394) \\ (580+580)\end{tabular}                & DNN                        & -         & -      & 0.79     & -        & -    \\
\hline
Yates et al.~\cite{yates2017depression}                                          & Reddit                                                                         & \begin{tabular}[c]{@{}c@{}}RSDD\\(9000+107000)\end{tabular}                                                                                                                                                                                                                                      & CNN                        & 0.75      & 0.57   & 0.65     & -        & -    \\
\hline
Orabi et al ~\cite{orabi2018deep}                                                & Twitter                                                                        & \begin{tabular}[c]{@{}c@{}}CLPsych2015 \\1145(327+572+246PTSD)\end{tabular}                                                                         & CNN                        & 0.87      & 0.87   & 0.87     & 0.88     & 0.95 \\
\hline
Orabi et al ~\cite{orabi2018deep}                                                & Facebook                                                                       & \begin{tabular}[c]{@{}c@{}}Bell Let’s Talk\\154(53+101)\end{tabular}                                                                                & CNN                        & 0.82      & 0.84   & 0.82     & 0.83     & 0.92 \\
\hline
Rao et al.~\cite{rao2020mgl}                                                     & Reddit                                                                         & \begin{tabular}[c]{@{}c@{}}RSDD\\(9000+107000)\end{tabular}                                                                                                                                              & CNN                        & 0.63      & 0.48   & 0.54     & -        & -    \\
\hline
Rao et al.~\cite{rao2020mgl}                                                     & Reddit           & \begin{tabular}[c]{@{}c@{}}eRisk2017\\(83+403/52+349)\end{tabular}                                                                                                                                          & CNN                        & 0.63      & 0.57   & 0.60     & -        & -    \\
\hline
Sadeque et al.~\cite{ sadeque2017uarizona   }                                    & Reddit                                                                         & \begin{tabular}[c]{@{}c@{}}eRisk2017\\(83+403/52+349)\end{tabular}                                                                                                                                                                 & SVM+RNN                    & 0.61      & 0.67   & 0.64     & -        & -                   \\
\hline
Hu et al.~\cite{hu2021depression}                                                 & Weibo                                                                          & Posts56910(28455+28455)                                                                                   & Bi-directional LSTM                    & -         & -      & -        & 0.95     & -    \\
\hline
\end{tabular}
		\begin{tablenotes}
			\footnotesize
			\item The social network data used in the research is usually analyzed by users, but some of the research is based on posts, which we have noted in the table.
		\end{tablenotes}
\end{table*}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Table\ref{tab5} summarizes the experimental results of the social platform text-based depression detection, including the source and name of the method, the type of data used and the source of the dataset, and the evaluation criteria of the experimental results including Precision, Recall, F1 Score, Accuracy, and AUC.
...
Most researchers used the social network data they collected to analyze depression. Based on the research on the use of public datasets, we can draw the following conclusions:
...
For the non-sequential model, Sadeque et al.~\cite{sadeque2017uarizona} uses a SVM, which can simultaneously observe the entire user's late history.
For the sequential model, they used an RNN model to observe each post of one user at a time. Finally, they use an ensemble model to combine both the sequential and non-sequential models, it  takes full use advantage of each model.
