...
Subjects match one option they appropriate in each item and the different range of overall score is divided to different degree of depression. 
...
%自评量表
Self-rating scale like BDI-II is a kind of self-assessment based on the instructions, which can be easily completed in 5-10 minutes but not quite reliable. 
...
In recent years, some research showes that abnormal behavioral signals like facial expressions, voice and gait can reflect mental disease, which is more easily to observe compared with physiological electrical signals or brain images.
...
By analyzing the abnormal behavioral signals can auxiliary diagnosing depression.

%%补充抑郁量表说明
In general, the data of patients with depression are not disclosed due to privacy issues, so most of the studies mentioned in this paper use the results of the scale as labels for experiments, and these scales are tests for the normal population who have not been diagnosed with depression.
...
This section introduces some public datasets as shown in Table \ref{tab2}, including the name, information of subject and data modality. Due to privacy issues, only several of the data modality has public dataset.
Therefore, most researchers use the data collected by themselves for analysis.

The Reddit Self-reported Depression Diagnosis (RSDD) dataset consists of Reddit posts for 9,210 users who have claimed to have been diagnosed with depression ("diagnosed users") and 107,274 healthy control users, which is a large-scale general forum dataset. The data is in JSON format, and each row is an array representing a user. The label field includes the user’s label (control or depression), and the posts field contains timestamp and untokenized pairs~\cite{yates2017depression}.

The early risk (eRsik) dataset consists of text examples collected from messages of 887 Reddit users. The main idea of the task is to classify users into risk case of depression and non-risk case~\cite{Stankevich2018FeatureEF}.
...
The Extended DAIC Database(E-DAIC) is the extended version of DAIC-WOZ database for depression and Post Traumatic Stress Disorder assessment, developed by ICT~\cite{gratch2014distress}. This data was used for the AVEC2019 Challenge.
...
The Emotional Audio-Textual Depression Corpus (EATD-Corpus) contains audios and extracted transcripts of responses from 162 depressed and non-depressed volunteers. EATD-Corpus is the first and only public depression dataset that includes audio and text data in Chinese.

Finally, we also present some private datasets that are collected by the researchers themselves and used only for their own analysis.
...
\begin{tabular}{c|c|c|c|c}
	...
	\hline
\textbf{ }      &\textbf{Dataset}                    & \textbf{Subjects (D+C)} & \textbf{Type}                & \textbf{Task}                     \\
\hline
...
In many studies, the diagnosis of depression is considered as a binary classification problem.
Subject with the minimal degree of the scale were labeled to have non-depressed and others are labeled to have depressed.
And the experimental evaluation methods were some common classification metrics such as Precision, Accuracy, Recall, F1-score, Specificity, Sensitivity and Area Under the Curve (AUC).
...
(1) Precision reflects the ability of the model to correctly predict positive samples, it calculates how many positive samples are predict as \emph{positive}. The calculation is formulated as:
...
(2) Accuracy reflects the ability of the model to correctly predict the overall sample, it calculates how many positive samples are predict as \emph{positive} and negative samples as \emph{negative}. The calculation is formulated as:
...
(3) Recall reflects the ability of the model to correctly predict the full degree of positive samples, it  calculates the proportion of positive samples predicted as \emph{positive} to the total positive samples. The calculation is formulated as:
...
(4) F1-score is a statistical measure of the accuracy of model. It defined as the harmonic mean of precision and recall. The calculation is formulated as:
...
(5) Specificity reflects the ability of the model to correctly predict the full extent of negative samples, it  calculates the proportion of negative samples predicted to be \emph{negative} to the total negative samples. The calculation is formulated as:
...
(6) Sensitivity reflects the ability of the model to correctly predict the full extent of positive samples, it  calculates the proportion of positive samples predicted to be \emph{positive} to the total positive samples. The calculation is formulated as:
...
(1) MAE reflects the mean absolute difference between the predicted values and the actual values in a dataset. The lower the MAE, the better a model fits a dataset. The calculation is formulated as:
...
(2) RMSE reflects the square root of the average squared difference between the predicted values and the actual values in a dataset. The lower the RMSE, the better a model fits a dataset. The calculation is formulated as:
...
\begin{figure}[tbp]
	\centering	
	\label{fig_hard_case1}\includegraphics[width=0.9\linewidth]{figures/depression/method.png}		
	\caption{
	The process of auxiliary depression diagnosis method based on different modality data. 
	}
	\label{method}
\end{figure}
...
Although various methods have been proposed to auxiliary depression diagnosis based on different multimodal data, the process of methods is similar, as shown in  Fig \ref{method}.

\subsubsection{Pre-processing} Most of the raw data have a lot of noises from capture device and environment.
...
\subsubsection{Feature extraction} Feature extraction means to the process of transforming raw data into numerical features that can be processed while preserving the information. 
...
Automated feature extraction uses specialized algorithms to extract features automatically from data without the need for human intervention, such as deep learning methods.
...
\subsubsection{Prediction} Auxiliary depression diagnostic models based on different modality data are divided into traditional machine learning methods such as Support Vector Machine (SVM), Support Vector Regression (SVR), Random Forests (RF), Logistic Regression (LR), and deep learning methods such as Convolutional Neural Networks (CNN), Deep Neural Networks (DNN), Recurrent Neural Network (RNN), Long short-term Memory (LSTM).
...
Since it can solve the problems encountered in traditional machine learning methods for human-crafted features, such as high threshold, labor expense and low feature utilization rate, deep learning methods are gradually gaining the upper hand in machine learning.
...
In the subsequent sections we present the analysis methods based on different modality data in more detail, reviewing both traditional machine learning and deep learning methods, and summarizing the experimental results.