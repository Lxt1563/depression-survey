...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
As the most commonly used tools, a series of questionnaires capturing depressive symptoms had been developed and showed great success in psychiatric practice. The standard depression test scales as shown in Table \ref{tab1}.
...
Measuring depression on a scale alone has limitations, so physiological electrical signals or brain images from subjects for auxiliary depression diagnosis during professional diagnosis.
In recent years, some research shows that abnormal behavioural signals like facial expressions, voice and gait can reflect mental disease, which is more easily observed than physiological electrical signals or brain images.
...
%%补充抑郁量表说明
In general, the data of patients with depression are not disclosed due to privacy issues, so most of the studies mentioned in this paper use the results of the scale as labels for experiments.
These scales are tests for the normal population who have not been diagnosed with depression.
...
And the experimental evaluation methods were some standard classification metrics such as Precision, Accuracy, Recall, F1-score, Specificity, Sensitivity and Area Under the Curve (AUC).
...
\begin{figure}[tbp]
	...
	\caption{
	Confusion Matrix: each row represents the forecast category, and the total number of each row represents the number of data predicted for that category. Each column represents the real category of data, and the total data in each column represents this category's number of data instances.
	}
	\label{metric}
\end{figure}
...
Although various methods have been proposed for auxiliary depression diagnosis based on different multimodal data, the process of methods is similar, as shown in Fig \ref{method}.
...
\begin{figure}[tbp]
	...
	\caption{
	The process of the auxiliary depression diagnosis method is based on different modality data. 
	}
	\label{method}
\end{figure}


\subsubsection{Pre-processing} Most of the raw data have a lot of noise from the capture device and environment.
...
\subsubsection{Feature extraction} Feature extraction means transforming raw data into numerical features that can be processed while preserving the information. 
Feature extraction includes manual or automatic.
Manual feature extraction requires researchers to manually extract features based on the characteristics of data and tasks. 
In many situations, a good understanding of the task can help make informed decisions about which features could be useful.
Automated feature extraction uses specialized algorithms to extract features automatically from data without human intervention, such as deep learning methods.
...
Because it can solve the problems of artificial features encountered in traditional machine learning methods, such as high threshold, labor cost, and low feature utilization, the depth learning method gradually dominates in machine learning.

%%%%%%%%%%% differernt data modality

Although the processes are similar, the auxiliary depression diagnostic methods based on various data modalities are not comparable because the data types, extraction features, and classification algorithms are different.
