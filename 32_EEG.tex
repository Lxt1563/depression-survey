...
EEG can detect these abnormal activities as a method of tracking brain function~\cite{hughes1999conventional}.
...
Actually, related research are shifting from early traditional methods based on hand-crafted features to implementing deep learning architectures.
...
Depression's electrophysiology covers EEG, ECG, electrodermal, gastric, EOG, and body temperature measurements. EEG is utilized the most commonly~\cite{liao2017major,bairy2017automated,acharya2018automated}, followed by ECG~\cite{ding2019classifying}. It is noteworthy that ECGs are common used to diagnose depression by extracting RR interval sequences and measuring HRV. In addition, as other types of signals are employed infrequently, we concentrate on EEG and HRV-based auxiliary depression diagnosis.
...
Feature extraction and classification are the two critical components of a traditional machine learning approach.
...
Typically, time domain analysis focuses on the measurement and analysis of the raw EEG signal waveform to calculate more intuitive or physiologically significant EEG properties.
...
Nonlinear approaches can capture the chaotic behavior and abrupt changes in EEG signals caused by brain-based physiological phenomena~\cite{rodriguez2015analysis}.
...
For instance, the correlation dimension indicates the system's complexity, the greatest Lyapunov exponent reflects the rate of uncertainty expansion and is sensitive to the system's initial value. The entropy quantifies the system's amount of order.
...
The results suggest that by judiciously combining nonlinear features, it may objectively and effectively depression diagnosis.
...
It is demonstrated that SVM has the highest classification accuracy compared to other approaches. Due to the fact that SVM has the most significant distance between the separation hyperplane and the kernel trick, it is more robust and computationally efficient, resulting in the most excellent accuracy.
...
Most of the hybrid neural networks are combined models of both CNN and LSTM blocks.
Additionally, it has two significant data input formats: time series and feature images.
Concretely, Thoduparambil et al.\cite{thoduparambil2020eeg} design a deep model in which an integration of CNN and LSTM is implemented to detect depression.
...
The frequency domain measures vagal modulation of the RRI derived from the electrocardiogram.
Frequency domain characteristics indicate numerous spectral components, including very low frequency (VLF: 0.0033C0.04 Hz), low frequency (LF: 0.04C0.15 Hz), and high frequency (HF: 0.15C0.4 Hz)~\cite{kidwell2018heart}.
%LF is modulated by sympathetic and parasympathetic activities;
%HF is modulated by parasympathetic activities;
Sympathetic and parasympathetic activities modulate LF;
Parasympathetic activities modulate HF;
...
%(VLF) (0.003C0.04 Hz), LF (0.04C0.15 Hz), HF (0.15C0.4 Hz). LF is modulated by sympathetic
...
Byun et al.~\cite{byun2019detection} demonstrate HRV-based recognition using the SVM classifier. Monitoring the changes in linear and nonlinear HRV features for various autonomic nervous system states can facilitate the more objective identification of depression patients.
...
For auxiliary depression diagnosis, Mohanraj et al.~\cite{mohanraj2022deep} propose a neural network called DesNN.
...
Firstly, single-channel EEG analysis in the traditional method, employing the combination of measures, can provide the accuracy for discrimination of depression not lower than reported in other studies where multichannel EEG signals were analyzed~\cite{bachmann2018methods}.
...
Acharya\cite{acharya2018automated} presented the first application of the deep neural network concept for the auxiliary depression diagnosis.
...
&  & &  &   Linear:73.30  \\
& & & \multirow{-2}{*}{KNN}  & Nonlinear80.00  \\ \cline{4-5}
& & & & Linear:76.60 \\
