...
However, fundamental linear indicators may not be adequate for describing the dynamic activities of complex interactions~\cite{wright1996dynamics}, and it is challenging to obtain a complete picture of the EEG by examining it just in the temporal domain.
...
%And their detailed flow is shown in Fig~\ref{eeg_cnn}.
...
The DesNN model consists of seven convolutional layers, five standard bunch layers, three pooling layers, and one fully convolutional layer.
The last completely associated layer uses softmax actuation work to the output predictions, whereas all other layers use cracked modified straight unit (LeakyReLU) enactment work.
...
&  & &  &   Linear:73.30  \\
& & & \multirow{-2}{*}{KNN}  & Nonlinear£º80.00  \\ \cline{4-5}
& & & & Linear:76.60 \\
