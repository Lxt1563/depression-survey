...
Besides, Saeedi et al.~\cite{saeedi2021major} use brain effective connectivity method to convert 1D EEG signal into a 2D image. Then, they develop a classifier using state-of-the-art deep learning methods (CNN-LSTM) as a novel approach for the automated recognition of depression patients from EEG signals.
...
Traditional machine learning, a mainstream paradigm in HRV depression recognition, tries to discover depression-related variables and expand the feature set to improve performance.
...
Byun et al.~\cite{byun2019detection} demonstrate the HRV-based recongnition using the SVM classifier. Monitoring the changes in linear and nonlinear HRV features for various autonomic nervous system states can facilitate the more objective identification of depression patients.
...
Concretely, to the depression recognition, Mohanraj et al.~\cite{mohanraj2022deep} propose a neural network called DesNN.
...
Anyhow, the experimental results indicate that the end-to-end deep learning approach can identify depression from ECG signals, and possess high recognition performance. It also shows that ECG is a potential biomarker in the diagnosis of depression.
...
We evaluate the depression recognition method using the EEG and ECG, two commonly utilized electrophysiological markers.
...
Acharya\cite{acharya2018automated} presented the first application of the deep neural network concept for the depression recognition.
