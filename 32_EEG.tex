...
Further, depressive episodes may be accompanied by nausea, vomiting, chest tightness, and sweating~\cite{faris2006evidence}.
...
Besides that, breathing plays a crucial role in the regulation of autonomic systems~\cite{zygmunt2010methods}.
...
(1)Traditional machine learning: it begins with feature extraction, then feature selection to acquire more effective and representative aspects of depression, and then creating prediction or clustering models based on these features to perform the classification problem~\cite{halilaj2018machine}.
...
Non-linear approaches are able to capture the chaotic behavior and abrupt changes in EEG signal caused by a brain-based physiological phenomena~\cite{rodriguez2015analysis}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
There is a frequent use of HRV time domain characteristics when evaluating the linear activity state of the cardiovascular system.
...
Likewise, Daniel et al.~\cite{vigo2004relation} found that compared to healthy controls, patients with depression above the age of 60 had significantly lower sample entropy values.


In order to recognize depression using HRV, a variety of classifiers can be used.
Kang et al. \cite{kuang2017depression} use the bayesian network to recognition depressed patients from a healthy people, the recognition results demonstrate the significant association between depression and HRV.
Byun et al.~\cite{byun2019detection} demonstrate the HRV-based diagnosis using SVM classifier. Monitoring the changes in linear and nonlinear HRV features for various autonomic nervous system states can facilitate the more objective identification of depression patients.
...
For HRV auxiliary depression diagnosis based on deep learning, scholars build an end-to-end deep architecture and then push raw signal into deep architecture to let model learn high-level features by itself.
...
The DesNN model contains 7 convolutional layers, 5 bunch standard layers, 3 pooling layers, and 1 fully convolutional layer.
...
adjacent information is interdependent. Normally, this interdependence is useful in predicting the future state.
In RNN, the layers are connected so that the recurrent process can take place.
LSTM is a form of RNN that can learn long-term relationships.
Specially, Noor et al.~\cite{noor2021predicting} present an RNN with an LSTM autoencoder to predict the risk of depression based on HRV.
...
A threshold value has been taken to predict normal and depression risky heartbeat.
...
We review the effectiveness of the approaches examined in Tabel~\ref{tab_eeg} in order to provide greater insight into the gait depression recognition methods' performance.
The experimental classification accuracy data (\%) are taken directly from the related source articles in order to conduct a fair comparison.
...
(1)EEG auxiliary depression diagnosis is undergone a transition from the early traditional methods to the application of architectures of deep learning. The performance comparisons based on traditional machine learning and deep learning
are summarized in Table~\ref{tab_eeg}.
To be specific, firstly, single-channel EEG analysis in traditional method, employing the combination of measures, can provide the accuracy for discrimination of depression not lower than reported in other studies where multichannel EEG signals were analysed~\cite{bachmann2018methods}.
Secondly, the nonlinear analysis of EEG can be a useful method for discriminating depressed patients and normal subjects.
...
Finally, in contrast, the novelty of the deep learning model is that it does not require the employment of feature extraction, selection, and reduction. The model has the ability to self-learn and pick up distinctive features during training without a separate feature extraction or feature selection step.
...
(2)
Current studies in which ECG/HRV features were used to discriminate depression patients have yielded promising results obtained via various machine learning methods.
Firstly, compared to those previous results, the highest accuracy achieved in ~\cite{byun2019detection} are relatively low. However, some of the previous studies reported the results based on a small number of samples~\cite{roh2014wearable} or they lacked the description of the prediction accuracy validation method~\cite{sun2016objective}. Without cross-validation, predictive performance can be over-estimated.
Secondly, the performance of all HRV deep learning algorithms is above 90\%, but because the datasets are different, there is no way to compare them in a fair and unbiased manner.
...
expressed from the RR interval collected from ECG data. It is inevitable that the QRS wave algorithm will be utilized to
...
In addition, traditional studies used simple classifiers to train features manually extracted from HRV sequences. The performance of this type of classifier mainly depends on the feature selection process, which is
laborious and time-consuming. The deep model can overcome the shortcomings of manual feature extraction and selection.


\begin{table}[!htbp]\large
\caption{ Overview of machine learning based methods for Depression
Assessment from electrophysiological signals.}
\center
\label{tab_eeg}
...
\toprule
\textbf{Modal} & \textbf{Paper} & \textbf{Dataset}  & \textbf{Classification}  & \textbf{Accuracy/Value}    \\ \midrule
& Acharya~\cite{acharya2015novel}   & 30 (15D+15C)   & SVM   & 98    \\ \cline{2-5}
 & {}   &   &     & Single-channel:92   \\
 & \multirow{-2}{*}{{Bachmnn~\cite{bachmann2018methods}}} & \multirow{-2}{*}{26 (13D+13C)} & \multirow{-2}{*}{LR}& Multil-channel:90  \\ \cline{2-5}
&  &   & KNN  & Linear/Nonlinear:73.3/80   \\ %\cline{2-5}
&  &   & LDA  & Linear/Nonlinear:76.6/86.6 \\ %\cline{2-5}
 & \multirow{-3}{*}{Hosseinifard~\cite{hosseinifard2013classifying}} & \multirow{-3}{*}{90 (45D+45C)} & LR  & Linear/Nonlinear:76.6/90   \\ \cline{2-5}
...
& \multirow{-7}{*}{Cukic~\cite{cukic2018eeg}} & \multirow{-7}{*}{43 (23D+20C)} & Naive Bayes & 92.68 \\ \cline{2-5}
& Ahmadlou~\cite{ahmadlou2012fractality}& 24 (12D+12C)  & EPNN   & 91.30  \\ \cline{2-5}
& Faust~\cite{faust2014depression} & 60 (30D+30C) & Probabilistic neural network & 99.7 \\ \cline{2-5}
 & Liao~\cite{liao2017major} & 24 (12D+12C)   & SVM  & 80  \\ \cline{2-5}
& Wan~\cite{wan2020hybrideegnet}   & 35 (23D+12C)  & HybridEEGNet   & 79.08   \\ \cline{2-5}
& Saeedi~\cite{saeedi2021major}& 64 (34D+30C)  & 1DCNN-LSTM  & 99.24  \\ \cline{2-5}
& &  &  & Left hemisphere:98.84      \\  %\cline{2-5}
 & \multirow{-2}{*}{Thoduparambil~\cite{thoduparambil2020eeg}}& \multirow{-2}{*}{}& \multirow{-2}{*}{CNN-LSTM}  & Right hemisphere:99.07 \\  \cline{2-5}
& & & & Left hemisphere:93.5  \\  %\cline{2-5}
& \multirow{-2}{*}{Acharya~\cite{acharya2018automated}}  & \multirow{-2}{*}{30 (15D+15C)} & \multirow{-2}{*}{CNN} & Right hemisphere:96.0 \\ \cline{2-5}
 & Kang~\cite{kang2020deep} & 64 (34D+30C) & CNN  & 98.85 \\ \cline{2-5}
& Sharma~\cite{sharma2021dephnn} & 45 (21D+24C)& DepHNN & 99.1 \\ \cline{2-5}
 & Li~\cite{li2019eeg} & 48 (24D+24C) & ConvNet & 85.62  \\ \cline{2-5}
...
\multirow{-27}{*}{EEG} & \multirow{-2}{*}{Ay~\cite{ay2019automated}}& \multirow{-2}{*}{30 (15D+15C)} & \multirow{-2}{*}{CNN-LSTM}& Right hemisphere:97.66   \\  \cline{1-5}
 & Zhang~\cite{zhang2011new}  & 10D+10C  & neuro-fuzzy network  & 95   \\ \cline{2-5}
& &  & SVM  & 74.4   \\    %\cline{2-5}
& \multirow{-2}{*}{Byuna~\cite{byun2019detection}} & \multirow{-2}{*}{72 (31D+41C)}  & Statistical filter & 73.1  \\ \cline{2-5}
 & Roh~\cite{roh2014wearable} & 23D  & SVM  & 71  \\ \cline{2-5}
& Matsui~\cite{matsui2016impaired} & 41 (13D+28C) & LDA  & 88 \\ \cline{2-5}
& Sun~\cite{sun2016objective}  & 91 (44D+47C) & LR  & 79   \\ \cline{2-5}
& Kuang~\cite{kuang2017depression} & 76 (38D+38C) & Bayesian & 86  \\ \cline{2-5}
& Noor~\cite{noor2021predicting} & 5000D & RNN  & 97.24     \\ \cline{2-5}
& zang~\cite{zang2022end} & 74 (37D+37C)  & CNN   & 93.96  \\ \cline{2-5}
\multirow{-10}{*}{HRV} & Mohanraj~\cite{mohanraj2022deep} & 30 (15D+15C) & DesNN& 90 \\ \bottomrule
\end{tabular}
\end{threeparttable}}
\end{table}
