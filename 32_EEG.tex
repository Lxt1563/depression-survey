...
These symptoms are associated with autonomic dysfunction, which can be substantially reflected by signals such as the ECG, heart sounds, and pulse.
...
Besides that, breathing plays a crucial role in regulating autonomic systems~\cite{zygmunt2010methods}.
...
Due to the non-invasive detection and easy use of electrophysiological signals, depression recognition with them has become a hot research topic and the most promising research direction.
...
Depression's electrophysiology covers EEG, ECG, electrodermal, gastric, EOG, and body temperature measurements. EEG is utilized the most commonly~\cite{liao2017major,bairy2017automated,acharya2018automated}, followed by ECG~\cite{ding2019classifying}. It is noteworthy that ECGs are commonly used to diagnose depression by extracting RR interval sequences and measuring HRV. In addition, as other types of signals are employed infrequently, we concentrate on EEG and HRV-based depression recognition.
...
Therefore, the EEG is one of the most promising signals that may be utilized for depression recognition.
...
In the study of EEG depression recognition, the time domain features generally used include the mean, variance, kurtosis, and skewness of the signal.
...
Nonlinear approaches can capture the chaotic behavior and abrupt changes in EEG signal caused by brain-based physiological phenomena~\cite{rodriguez2015analysis}.
...
To verify the conclusion mentioned above, Acharya et al.~\cite{acharya2015novel} present a novel method for automated depression recognition based on nonlinear techniques: fractal dimension, largest Lyapunov exponent, sample entropy, detrended fluctuation analysis, Hurst's exponent, higher order spectra, and recurrence quantification analysis.
The results suggest that by judiciously combining nonlinear features, it may objectively and effectively depression recognition.
...
In addition, Mahato et al.~\cite{mahato2019electroencephalogram} examine numerous EEG signal analysis approaches and recommend the best accurate technique for depression recognition.
...
Time-domain analysis with moving window segmentation is employed to generate input data to extract the statistical mean feature.
...
Besides, Saeedi et al.~\cite{saeedi2021major} use brain effective connectivity method to convert 1D EEG signal into a 2D image. Then, they develop a classifier using state-of-the-art deep learning methods (CNN-LSTM) as a novel approach for the automated diagnosis of depression patients from EEG signals.
...
%        \item Alpha power variability: APV; Relative gamma power: RGP; Detrended fluctuation analysis: DFA; Linear discriminant analysis: LDA; Sample Entropy: SampEn; % 锟斤拷锟接此达拷
...
Physiological variation of the interval between consecutive heartbeats is known as HRV. HRV analysis is traditionally performed on ECG signals and has become a helpful tool in depression recognition~\cite{heart1996standards}.
...
In a recent study, there are two overall strategies for HRV-based depression recognition: traditional methods with hand-crafted features and end-to-end deep learning methods.
...
In HRV depression recognition, the time domain features commonly used include:
...
%(VLF) (0.003–0.04 Hz), LF (0.04–0.15 Hz), HF (0.15–0.4 Hz). LF is modulated by sympathetic
...
Nonlinear dynamics and information theory have been recommended as a modification for HRV analysis because heart rate fluctuation is a complicated behavior resulting from nonlinear regulatory systems~\cite{goldberger2002fractal}.
...
Byun et al.~\cite{byun2019detection} demonstrate the HRV-based diagnosis using the SVM classifier. Monitoring the changes in linear and nonlinear HRV features for various autonomic nervous system states can facilitate the more objective identification of depression patients.
...
For HRV depression recognition based on deep learning, scholars build a deep end-to-end architecture and then push raw signals into deep architecture to let the model learn high-level features.
Specifically, they can be subdivided into two categories depending on the model used: CNN and Recurrent Neural Network (RNN).
...
Concretely, to the auxiliary diagnosis of depression, Mohanraj et al.~\cite{mohanraj2022deep} propose a neural network called DesNN.
...
Similarly, Zang et al.~\cite{zang2022end} propose an end-to-end CNN network containing two convolution layers, two max-pooling layers, and one fully-connected layer. The activation function of the convolutional layer is the rectified linear units (RELU). After each convolution layer, max-pooling is applied to the obtained feature maps. The Softmax function of the fully-connected layer outputs normal or depression.
Anyhow, the experimental results indicate that the end-to-end deep learning approach can identify depression from ECG signals, and possess high diagnostic performance. It also shows that ECG is a potential biomarker in the diagnosis of depression.


%The hybrid neural network are combined models of both Recurrent Neural Network (RNN) and LSTM blocks.
...
In RNN, the layers are connected so the recurrent process can occur.
LSTM is a form of RNN that can learn long-term relationships.
Especially, Noor et al.~\cite{noor2021predicting} present an RNN with an LSTM autoencoder to predict the risk of depression based on HRV.
The RNN learns the signals from the dataset and passes those signals to the LSTM autoencoder. There are two parts to the auto-encoder architecture in common. The encoder compresses the input, the decoder attempts to recreate it, and the recurrent auto-encoder joins the encoder and decoder, capturing the regenerated ECG signals.
The training process is done based on those signals.
A threshold value has been taken to predict normal and depressed risky heartbeat.
The excellent experimental results demonstrate that RNNs are superior to CNNs for processing HRV signals because they are naturally memory-rich.
...
We review the effectiveness of the approaches examined in Tabel~\ref{tab_eeg} to provide greater insight into the gait depression recognition methods' performance.
...
(1) EEG depression recognition has transitioned from the early traditional methods to the application of deep learning architectures. The performance comparisons based on traditional machine learning and deep learning are summarized in Table~\ref{tab_eeg}.
...
Acharya\cite{acharya2018automated} presented the first application of the deep neural network concept for the diagnosis of depression.
...
(2)
Current studies using ECG/HRV features to discriminate depression patients have yielded promising results obtained via various machine learning methods.
Firstly, compared to those previous results, the highest accuracy achieved in ~\cite{byun2019detection} is relatively low. However, some of the previous studies reported the results based on a small number of samples~\cite{roh2014wearable}, or they needed the description of the prediction accuracy validation method~\cite{sun2016objective}. With cross-validation, predictive performance can be overestimated.
Secondly, the performance of all HRV deep learning algorithms is above 90\%, but because the datasets are different, there is no way to compare them fairly and unbiasedly.
%Here, we just list their advantages.
Here, for this application, we just discuss the properties of traditional machine learning methods and deep learning methods.
Traditional studies have used HRV features to discriminate patients with depression, and HRV is usually
expressed from the RR interval collected from ECG data. The QRS wave algorithm will inevitably be utilized to
locate R-peak. The R-peak positioned by different QRS wave algorithms also has biases. On the contrary, the deep learning
method directly takes the ECG signal as input without extracting the HRV sequence.
In addition, formal studies used simple classifiers to train features manually extracted from HRV sequences. The performance of this type of classifier mainly depends on the feature selection process, which is laborious and time-consuming. The deep model can overcome the shortcomings of manual feature extraction and selection.
...
\begin{tabular}{@{}c|c|c|c|c@{}}
\toprule
\textbf{Type}& \textbf{Paper} & \textbf{Dataset} & \textbf{Methods} & \textbf{Metrics(Accuracy/\%)} \\ \midrule
& Acharya et al~\cite{acharya2015novel}  & 30(15D+15C)  & SVM  & 98.00     \\ \cline{2-5}
& & &  & S-channel:92.00  \\
& \multirow{-2}{*}{{Bachmnn et al~\cite{bachmann2018methods}}} & \multirow{-2}{*}{26(13D+13C)} & \multirow{-2}{*}{LR} & M-channel:90.00 \\ \cline{2-5}
&  & &  &   Linear:73.30  \\
& & & \multirow{-2}{*}{KNN}  & Nonlinear：80.00  \\ \cline{4-5}
& & & & Linear:76.60 \\
& & & \multirow{-2}{*}{LDA}  & Nonlinear:86.60   \\ \cline{4-5}
& &  &   & Linear:76.60                   \\
& \multirow{-6}{*}{Hosseinifard et al~\cite{hosseinifard2013classifying}}  & \multirow{-6}{*}{90(45D+45C)} & \multirow{-2}{*}{LR}& Nonlinear:90.00  \\ \cline{2-5}
& & & MLP  & 95.12 \\
& & & LR& 97.56  \\
& & & SVM& 95.12  \\
& & & Decision tree & 95.12    \\
...
& \multirow{-7}{*}{Cukic et al~\cite{cukic2018eeg}}& \multirow{-7}{*}{43(23D+20C)} & Naive Bayes & 92.68   \\ \cline{2-5}
& Ahmadlou et al~\cite{ahmadlou2012fractality} & 24(12D+12C) & EPNN  & 91.30  \\ \cline{2-5}
& Faust et al~\cite{faust2014depression} & 60(30D+30C) & PNN & 99.70\\ \cline{2-5}
& Liao et al~\cite{liao2017major} & 24(12D+12C) & SVM & 80.00   \\ \cline{2-5}
& Wan et al\cite{wan2020hybrideegnet} & 35(23D+12C) & HybridEEGNet   & 79.08 \\ \cline{2-5}
& Saeedi et al~\cite{saeedi2021major} & 64(34D+30C) & 1DCNN-LSTM  & 99.24 \\ \cline{2-5}
& &  &  & LH:98.84   \\
& \multirow{-2}{*}{Thoduparambil et al~\cite{thoduparambil2020eeg}}& \multirow{-2}{*}{}& \multirow{-2}{*}{CNN-LSTM}   & RH:99.07 \\ \cline{2-5}
& & & & LH:93.50  \\
& \multirow{-2}{*}{Acharya et al~\cite{acharya2018automated}} & \multirow{-2}{*}{30(15D+15C)} & \multirow{-2}{*}{CNN} & RH:96.00 \\ \cline{2-5}
& Kang et al~\cite{kang2020deep} & 64(34D+30C)& CNN & 98.85   \\ \cline{2-5}
& Sharma et al~\cite{sharma2021dephnn} & 45(21D+24C) & DepHNN  & 99.10  \\ \cline{2-5}
& Li et al~\cite{li2019eeg} & 48(24D+24C) & ConvNet  & 85.62   \\ \cline{2-5}
& & &  & LH:99.12    \\
\multirow{-30}{*}{EEG} & \multirow{-2}{*}{Ay et al~\cite{ay2019automated}} & \multirow{-2}{*}{30(15D+15C)} & \multirow{-2}{*}{CNN-LSTM}  & RH:97.66\\ \cline{1-5}
& Zhang et al~\cite{zhang2011new}& 20(10D+10C)  & FNN& 95.00  \\ \cline{2-5}
& &   & SVM  & 74.40     \\
& \multirow{-2}{*}{Byuna et al~\cite{byun2019detection}}& \multirow{-2}{*}{72(31D+41C)}& Statistical filter & 73.10 \\ \cline{2-5}
& Roh et al~\cite{roh2014wearable}& 23D & SVM & 71.00 \\ \cline{2-5}
& Matsui et al~\cite{matsui2016impaired} & 41(13D+28C)& LDA  & 88.00   \\ \cline{2-5}
& Sun et al~\cite{sun2016objective} &91(44D+47C)& LR & 79.00\\ \cline{2-5}
& Kuang et al~\cite{kuang2017depression} & 76(38D+38C) & Bayesian  & 86.00    \\ \cline{2-5}
& Noor et al~\cite{noor2021predicting} & 5000D  & RNN    & 97.24    \\ \cline{2-5}
& zang et al~\cite{zang2022end} & 74(37D+37C)   & CNN      & 93.96     \\ \cline{2-5}
\multirow{-10}{*}{HRV} & Mohanraj et al~\cite{mohanraj2022deep}  & 30(15D+15C)    & DesNN   & 90.00       \\ \bottomrule
\end{tabular}
\end{threeparttable}}
		\begin{tablenotes}
			\footnotesize
			\item Single-channel (S-channel), Multil-channel (M-channle), Multilayer per-
            \item ceptron (MLP), Probabilistic neural network(PNN),Fuzzy Neural Net-
            \item works(FNN).
		\end{tablenotes}
