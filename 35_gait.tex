...
It makes it possible to gather gait information and create models for gait auxiliary depression diagnosis.
There are two basic approaches in related work:
(1) \emph{Traditional machine learning.}
The key task in the traditional method is to learn gait characteristics from depression-related skeletal coordinates captured by Kinect and explore the feature set for better performance.
In the meantime, traditional classifiers, such as SVM, KNN, RF, etc., are employed for classification.
(2) \emph{Deep learning.}
Deep learning-based methods employ deep learning networks to replace the human feature extraction process with end-to-end models, which require only the raw data to make predictions.
Notably, these methods concentrate on two tasks: regression of scale scores and depression classification.
We concentrate primarily on the classification task because of the modest number of regression tasks involved.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The skeleton sequence refers to the 3D coordinate data that can characterize the human body behavior, which is abstracted from the human body structural model of each important joint of the human body~\cite{nordin2016survey}.
...
The extraction of silhouette is usually obtained by background subtraction and followed by binarization. Gait silhouette removes all the color and texture information and makes recognition focus on gait not clothing or other non-gait features.
The gait energy image (GEI)~\cite{han2005individual}, which is created by averaging the binary silhouette of the human body, has become a popular silhouette feature due to its straightforward processing and potent anti-noise properties~\cite{yang2008gait}.
...
In traditional research of gait auxiliary depression diagnosis, hand-crafted features are regularly used together with machine learning classifiers. The commonly used gait features are divided into two main categories: time-frequency, and spatial-temporal features.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Since the gait data is simply samplings of time series, the time-frequency analysis in signal processing is the method for identifying features describing gait. 
...
Specifically, there are four commonly used gait time-domain features, which are mean, standard deviation, skewness, and kurtosis of the original data.
...
In the meantime, several features in the frequency domain, such as amplitudes of all component frequencies, phases, etc., can be obtained with the help of the Fast Fourier transform (FFT). FFT can also be used to keep the key components while reducing the dimensionality of the gaits data. These temporal and frequency data have a strong correlation with depression, and some techniques, such SVM, etc., can use them as classification features to assist in the diagnosis of depression.
...
However, significant time- and frequency-domain principal components can only provide they with mathematical relationships
between gait patterns and time- and frequency- domain features. they are not intuitive for interpreting gait patterns.


Differently, while high-level spatial-temporal features may provide an intuitive understanding
of individual gait patterns, they contribute less to depression recognition than low-level time-and frequency-domain features~\cite{wang2021detecting}.
The spatiotemporal gait features, which basically comprise the distance between joints, the angle created by the joints, and the distance between the joint point and the bone, represent changes in the distance angle and other properties of the joints with time.
...
Applying these features to classifiers with various machine learning techniques, Fang et al.~\cite{fang2019depression} investigate whether natural gait analysis might be a practical and reliable approach to support depression recognition.
The results suggest that spatial features are very helpful in assessing depression.
Furthermore, a reasonable accuracy can already be achieved by using parameters from the upper body alone, indicating that upper body postures and movements can effectively contribute to depression analysis.
...
Depending on the input data, methods for gait auxiliary depression diagnosis using deep learning can group into two categories.
One is a deep end-to-end model with a skeleton sequence as input, and the other is the deep hybrid model with a skeleton sequence and silhouette as input.
...
LSTM and Temporal Convolutional Network (TCN) are two of the most widely used depth models today.
In the LSTM, the recurrent structure inside the network is constructed by taking the previous time's output as the current time's input, which is an efficient way to handle sequential data.
...
Yang et al.~\cite{yang2022data} use LSTM and TCN for auxiliary depression diagnosis, and while they produce good results, both LSTM and TCN, which focus solely on inter-frame information, lack spatial information construction.
In order to get around this limitation and effectively gather spatio-temporal information from 3D skeletons, Graph Convolutional Networks (GCN) can be used for skeleton data processing in the future.
GCN is the most realistic representation of human skeletal joints since it is based on the topological map approach.
...
The experimental results imply that the proposed method leveraging the complementarity gained a higher accuracy. However, the feature extraction of the skeleton alone and the generation of GEI with silhouettes suffer from variable degrees of information loss. Instead, employing a deep end-to-end architecture to push the original signal into the model for learning may produce unanticipated results.
...
We provide a performance summary of the methods evaluated in Tabel~\ref{tab_gait} in order to provide additional insight into the effectiveness of gait auxiliary depression diagnosis approaches.
The experimental classification accuracy (\%) are taken directly from the related source articles in order to conduct a fair comparison.
...
Wang~\cite{9187648}                  & 95(43D+52C)                   & FC  & 93.75  \\ \cline{1-4}
Yuan~\cite{yuan2018depression}  & 101(54D+47C) & SVM  & 91.09 \\ \cline{1-4}
Wang~\cite{wang2021detecting} & 247(126D+121C) & SVM  & AUC=0.93 \\ \cline{1-4}
Zhao~\cite{zhao2019see}    & 179                           & SVR   & 64  \\ \cline{1-4}
\multirow{4}{*}{Li~\cite{li2021simple}}   & \multirow{4}{*}{179(85D+85C)} & SVM  & 53.85   \\
                      &                               & RF  & 61.54    \\
                      &                               & LR  & 73.08   \\
                      &                               & GB  & 76.92    \\ \cline{1-4}
\multirow{3}{*}{Lu~\cite{lu2022postgraduate}}   & \multirow{3}{*}{95(43D+52C)}  & KNN & 80.34  \\
                      &                               & SVM & 91.21  \\
                      &                               & RF  & 85.71  \\ \cline{1-4}
\multirow{5}{*}{Fang~\cite{fang2019depression}} & \multirow{5}{*}{95(43D+52C)}  & SVM  & 90.53                         \\
                      &                               & RF  & 91.58                         \\
                      &                               & KNN  & 87.37                         \\
                      &                               & LR    & 88.42                         \\
                      &                               & LDA  & 88.42    \\ \cline{1-4}
Shao~\cite{shao2021multi}  & 200(86D+114C)                 & LSTM+CNN         & 85.45 \\ \cline{1-4}
\multirow{2}{*}{Yang~\cite{yang2022data}} & \multirow{2}{*}{95(43D+52C)}  & LSTM   & 90.35 \\
