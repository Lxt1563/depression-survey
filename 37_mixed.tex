...
\section{Auxiliary depression diagnosis method based on multiple audiovisual and text data}
...
Most depression recognition methods based on multimodal data mostly use audio, video and text data, because the international competition AVEC introduced audiovisual depression analysis in 2013, and compared with other modality data, it is easy to collect.
...
There are three methods to fuse modality data for depression recognition: early fusion, late fusion and hybrid fusion.
...
\subsection{Multiple audiovisual and text auxiliary depression diagnosis based on traditional machine learning}
\label{sec_fquality}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Machine learning methods play a key role in multimodal depression recognition.
As mentioned in the previous section, the method of depression recognition based on traditional machine learning audio, video and text focuses on feature extraction, and selects more discriminating features to predict the results.
...
A large number of experimental results show that, compared with single mode data, multimodal data combined with depression recognition has a better effect.

%\subsubsection{Audiovisual cues auxiliary depression diagnosis based on deep learning}


\subsection{Multiple audiovisual and text auxiliary depression diagnosison based on deep learning}
\label{sec_fquality}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Numerous previous studies have employed LSTM to extract spatiotemporal features at various scales from audio, video, and text clips to obtain crucial temporal information for a task involving depression recognition.
...
In recent years, hybrid models have attracted considerable attention in research employing audiovisual data for depression recognition.
...
The current study uses CNN and a transformer to analyze audio-video and text data.
For modeling audio video, scholars used CNNs to perform convolutions over the time dimension for the participants.
...
\hline
\multirow{2}{*}{\textbf{Paper}} & \multirow{2}{*}{\textbf{Dataset}} & \multirow{2}{*}{\textbf{Method}}& \multirow{2}{*}{\textbf{Features}} & \multicolumn{3}{c|}{\textbf{Classification metrics}} & \multicolumn{2}{c}{\textbf{Regression metrics}} \\
&  & & & {\textbf{F1 Score}} & {\textbf{Precision}} & {\textbf{Recall}} & {\textbf{RMSE}}  & {\textbf{MAE}} \\ \hline
...
Pampouchidou et al.~\cite{2017Facial} & AVEC2013/14 & Nearest Neighbour Classifier & A+V  & 73.00 & 95.00& 59.00 & -                 & -   \\ \hline
...
Williamson et al.~\cite{2016Detecting}& DAIC-WOZ& Gaussian Staircase Model & A+V+T& 81.00& - & - & 5.31  & 4.18 \\\hline
Gong et al.~\cite{gong2017topic} & DAIZ-WOZ& \begin{tabular}[c]{@{}c@{}}Stochastic Gradient Descent\\ Regression\end{tabular}                                                                                                                          & A+T+V & 60.00& - & - & 4.99 & 3.96 \\\hline
\multirow{3}{*}{Morales et al.~\cite{Morales2018ALF}} & \multirow{3}{*}{DAIC-WOZ} & \multirow{3}{*}{SVM}  & A+T                       &49.00 &36.00 & 78.00   & - & - \\
& & & A+V & 49.00&36.00 & 78.00 & -  & -  \\
& & & A+T+V & 49.00 & 36.00 &78.00  & - & -  \\\hline
Alhanai et al.~\cite{al2018detecting}& DAIC-WOZ& LSTM & A+T & 77.00& 71.00 & 83.00 & 6.37 & 5.10 \\\hline
\multirow{2}{*}{Rohanian et al.~\cite{rohanian2019detecting}}  & \multirow{2}{*}{DAIC-WOZ} & \multirow{2}{*}{LSTM}  & T+A & 80.00 & 78.00 & -  & 5.14 & 3.66   \\
& &  & A+V+T  & 81.00& 80.00 & - & 4.99 & 3.61  \\\hline
\multirow{4}{*}{Qureshi et.al.~\cite{qureshi2019verbal}} & \multirow{4}{*}{DAIC-WOZ}  & \multirow{4}{*}{LSTM}  & A+V                       & -  & - & - & 5.25 & 3.89  \\
& & & V+T& - & - & - & 5.11 & 3.65 \\
&  &  & A+T & - & - & -& 4.64 & 3.65  \\
& & & A+V+T & - & - & - & 4.14  & 3.07  \\\hline
Lau et al.~\cite{lau2021improving} & DAIC-WOZ & BiLSTM & A+T &92.00 & 100.00 & 86.00 &3.95 &3.00\\\hline
...
Yang et al.~\cite{yang2018integrating}  & DAIC-WOZ & DCNN+DNN & A+V & 60.00 & 55.00& 67.00& 6.34  & 5.39 \\ \hline
Lam et al.~\cite{lam2019context} & DAIC-WOZ & CNN+Transformer & A+T & 87.00 & 91.00 & 83.00 & - & -  \\\hline
Niu et al.~\cite{niu2021hcag} & DAIC-WOZ & Graph Attention Network & A+T & 92.00 & 92.00 & 92.00 &3.80 &2.94\\\hline
...
(2) On the AVEC 2013/2014 dataset, SVR achieves better results in traditional machine learning based methods.
...
For the video modality , an epsilon-SVR with intersection kernel trained using  Local Gabor Binary Pattern Three Orthogonal Planes features has been employed.
SVR with linear kernel and Sequential Minimal Optimization training as implemented in Waikato Environment for Knowledge Analysis is used for the audio baseline, and feature set consists of Low level descriptors related to energy, spectral, voicing, etc.
Finally, in order to create the audio-visual baseline, predictions from both audio and video baselines are obtained by taking the mean value for audio and video.
Valstar  ~\cite{10.1145/2661806.2661807} believes that the good results of depression recognition may be attributed to the following reasons:
...
Author observe that with the increase of feature numbers, Stochastic Gradient Descent and SVR models continually improve their performance while the random forest model stops improving much earlier.
...
Most of the data of other modes in the depression recognition use single mode data, but a few studies combine data of multiple modes for prediction.
...
Therefore, Mousavian et al~\cite{mousavian2021depression}. combines the similarity of spatial cubes of sMRI and fMRI data, extract features from them and input them into a unified machine learning classifier framework to detect depression.
Besides, several other modal fusions have also attracted extensive attention from researchers.
For example,
Zhang et al.~\cite{zhang2019multimodal} explores from physiological and behavioral perspectives simultaneously and fuse pervasive EEG and speech signals to make the detection of depression more objective, effective, and convenient.
Jing et al.~\cite{jing2019different} builds a supervised regression model based on individual language and natural gait data to predict depression and anxiety in patients. Their results could be the basis of both applications and future studies on multi-source data fusion in anxiety and depression recognition.
