...
To alleviate the damage caused by depression, an effective diagnosis is essential. However, there are three main challenges in the clinical diagnosis of depression:
(1) \emph{Insufficient objective indicators:}
Unlike other physiological diseases, there are no objective and precise biomarkers to assess depression, such as biochemical test indicators, physiological data, or medical imaging.
...
diagnosing and treating depression. Up to 75\% of patients do not obtain early diagnosis and treatment, especially in low-income and middle-income countries~\cite{baidu_seg}.
...
Because the general public has low awareness of depression, there is an underlying resistance or avoidance of mental illness.
Given this, early signs of depression may be ignored, losing the ideal window for treatment, which will make the condition worse and make therapy more challenging~\cite{finch2000perceived,lauber2005recommendations,angermeyer2011biogenetic,pescosolido2013backbone}.
...
\includegraphics[width=1\linewidth]{figures/depression/Taxonomy.eps}
\caption{The taxonomy of auxiliary depression diagnosis with data modality. We also show raw data with these data modalities: $a-f$ represent, EEG, HRV, brain imaging, facial expression, speech, and gait.
 }
...
%\begin{figure}
%\centering
%\includegraphics[width=1\linewidth]{figures/depression/ta.eps}
%\caption{The taxonomy of auxiliary depression diagnosis with data modality. We also show raw data with these data modalities: $a-f$ represent, EEG, HRV, brain imaging, facial expression, speech, and gait.
% }
%%\caption{The taxonomy of auxiliary diagnosis of depression from the perspective of various categories of data.}
%\label{Taxonomy}
...
To address the difficulty of clinical diagnosis of depression, researchers propose massive machine learning approaches in recent years.
...
(1) \emph{High efficiency:} by using computer technology to analyze the physiological signals of depressed patients and create objective diagnostic indicators, clinical diagnosis can make much more quickly and effectively.
(2) \emph{Low interference:} the automatic auxiliary depression diagnosis uses a machine learning method, which eliminates the need for professionals to observe consciousness.
Moreover, it lessens the difficulties of diagnosing and treating depression owing to patient resistance.


In the early stage of the studies, there were many traditional machine learning methods for better performance.
With the development of deep learning techniques, several new methods gradually emerged, and some breakthroughs have been made.
...
As shown in Fig\ref{Taxonomy}, we focus on the progress of the methods and potential research directions in four data modalities:~\emph{electrophysiological
...
Fig~\ref{Taxonomy} shows the representation forms of the corresponding physiological signals, which we will briefly describe in the following.
...
Electroencephalography (EEG), electrocardiography (ECG), and electrooculogram (EOG) are common electrophysiological signals relevant to clinical interest.
...
Studies have shown that, to some extent, brain imaging can identify different types of depression according to the part of the brain affected~\cite{nestler2002neurobiology}.
...
Depression's clinical manifestations include slower movement and speech~\cite{sobin1997psychomotor}, which can be caught audio-visually and serve as a valuable tool for an auxiliary depression diagnosis.
%{\color{red}
...
%本文围绕上述五种数据模式，综述了机器学习在抑郁症辅助诊断中的研究现状。我们重点关注机器学习的传统方法和先进的深度学习方法，全面介绍了机器学习技术在这些数据模式中的应用进展和潜在的研究方向，。据我们所知，这是第一个关于不同数据模式下辅助抑郁症诊断的全面调查， 这将为研究人员和临床心理学家提供更好的理解机器学习在这一领域的应用。

%本文围绕上述五种数据模式，综述了目前广泛应用于抑郁症辅助诊断的两种方法:深度学习方法和基于机器学习的传统方法。我们重点总结了机器学习在抑郁症辅助诊断领域的通用研究流程及常用研究方法，并比较了不同方法的优缺点。最后，结合当前智能计算和大数据技术的发展，讨论了未来发展的可能方向。
...
%In this paper, we review the current state of research on machine learning in auxiliary depression diagnosis regarding the five data modalities mentioned above. We focus on two methods that are currently widely used in depression-assisted diagnosis: deep learning methods and traditional methods based on machine learning. We summarizing the general research process and common research methods in the field of machine learning in depression-assisted diagnosis and compare the advantages and disadvantages of different methods. Finally, we discussed possible directions for future development in the context of the current development of intelligent computing and big data technologies.
%To the best of our knowledge, this is the first comprehensive survey about auxiliary depression diagnosis on different data modalities, which will provide researchers and clinical psychologists with a better understanding of the use of machine learning in this area.



In this paper, we review the current state of research on machine learning in auxiliary depression diagnosis regarding the four data modalities mentioned above.
We focus on two widely used methods in auxiliary depression diagnosis: traditional machine learning and deep learning.
We summarize the general research process and typical research methods in machine learning and compare the advantages and disadvantages of different methods. Finally, we discuss possible directions for future development in the context of the current development of intelligent computing and big data technologies.
To the best of our knowledge, this is the first comprehensive survey about auxiliary depression diagnosis on different data modalities, which will provide researchers and clinical psychologists with a better understanding of the use of machine learning in this area.
...
insights into promising research directions, while Section 9 concludes the survey.
